...
col_split_vec_conc <- ifelse(
  grepl("^PBMC \\| ", colnames(concordant_heatmap_mat)),
  "PBMC",
  ifelse(grepl("^DUODENUM \\| ", colnames(concordant_heatmap_mat)), "Duodenum", NA)
)

col_split_vec_conc <- factor(
  col_split_vec_conc,
  levels = c("PBMC", "Duodenum")
)

heatmap_concordant_pdf <- save_heatmap_pdf(
  mat          = concordant_heatmap_mat,
  filename     = "heatmap_top10_both_up_and_down.pdf",
  title        = "Top 10 pathways Both Up and Top 10 pathways Both Down\n(cell types paired by column)",
...
top_concordant <- matched %>%
  ...
  slice_max(order_by = concordance_score, n = 10, with_ties = FALSE) %>%
  ungroup()
...
heatmap_concordant_pdf <- save_heatmap_pdf(
  mat       = concordant_heatmap_mat,
  filename  = "heatmap_top10_both_up_and_down.pdf",
  title     = "Top 10 pathways Both Up and\n Top 10 pathways Both Down",
  row_split = row_split_vec
)
...
# =============================================================================
col_split_vec_disc <- ifelse(
  grepl("^PBMC \\| ", colnames(discordant_heatmap_mat)),
  "PBMC",
  ifelse(grepl("^DUODENUM \\| ", colnames(discordant_heatmap_mat)), "Duodenum", NA)
)

col_split_vec_disc <- factor(
  col_split_vec_disc,
  levels = c("PBMC", "Duodenum")
)

print(length(col_split_vec_disc))
print(ncol(discordant_heatmap_mat))
...
top_discordant <- matched %>%
 ...
 # group_by(direction_group, geneset) %>%
  group_by(direction_group) %>%
  slice_max(order_by = discordance_score, n = 10, with_ties = FALSE) %>%
...
heatmap_discordant_paired_pdf <- save_heatmap_pdf(
  ...
  title           = "Top 10 pathways PBMC Up / Duodenum Down +\n Top 10 Duodenum Up / PBMC Down\n(cell types paired by column)",
...
p_dot_concordant <- ggplot(
  ...
  scale_color_gradient2(low = "#2166AC", mid = "white", high = "#D6604D", midpoint = 0) +
  labs(
    title = "Top 10 concordant up and top 10 concordant down pathways",
    x = NULL,
    y = NULL,
    size = "-log10(FDR)",
    color = "NES"
...
panel_info <- list(
  ...
  list(file = dot_concordant_pdf, label = "C", title = "Top 10 concordant up/down pathways"),
