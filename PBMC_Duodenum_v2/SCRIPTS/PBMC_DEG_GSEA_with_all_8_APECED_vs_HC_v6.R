...
#   4) mild untreated APECED vs HC using direct subject-level pseudobulk
#   5) all APECED (mild untreated + severe) vs HC using direct subject-level pseudobulk
#
# GSEA databases:
#   - Hallmark
...
SCRIPT_VERSION <- "2026-07-15-v6-reproducible"
message("Loaded PBMC DEG/GSEA script version: ", SCRIPT_VERSION)

# =============================================================================
# REPRODUCIBILITY
# =============================================================================
REPRODUCIBLE_SEED <- 12345L
...
message("Reproducibility seed: ", REPRODUCIBLE_SEED)
message("BiocParallel mode: serial")
...
# FIXED: returns plain named numeric vector, not array
symbol_to_entrez_ranked <- function(gene_list_symbol) {
  ...
  # keep strongest symbol-level mapping
  map2 <- map2 %>%
    dplyr::group_by(SYMBOL) %>%
    dplyr::slice_max(order_by = abs(stat), n = 1, with_ties = FALSE) %>%
    dplyr::ungroup()
  
  # keep strongest entrez-level mapping
  map2 <- map2 %>%
...
build_pseudobulk <- function(obj,
  ...
  # Defensive input handling in case this helper is called directly.
  if (is.character(obj) && length(obj) == 1) {
    if (!file.exists(obj)) {
      stop("The supplied object path does not exist: ", obj)
    }
    
    log_msg("INPUT", paste0("Reading Seurat object from: ", obj))
    obj <- readRDS(obj)
  }
  
  if (!inherits(obj, "Seurat")) {
    stop(
      "`obj` passed to build_pseudobulk() must be a Seurat object or an ",
      "RDS filepath containing one. Received class: ",
  ...
  md <- md %>%
    ...
    dplyr::mutate(
      sample_id = if (include_pool_in_sample_id) {
        paste(subject_id, Pool, group, CellType, sep = "__")
      } else {
        paste(subject_id, group, CellType, sep = "__")
      }
    )
...
run_gsea_for_deg <- function(tt, out_dir, prefix, top_n_plot = 15) {
  ...
  log_line("Reproducibility seed: ", REPRODUCIBLE_SEED)
  log_line("BiocParallel: SerialParam")
...
run_paired_analysis <- function(ct, ct_counts, ct_meta, out_dir, gsea_dir) {
  ...
  # A cell type can be absent from one treatment group after pseudobulk filtering.
  # Add missing columns so the paired analysis skips cleanly instead of erroring.
  if (!"Mild_Untreated" %in% colnames(pair_table)) {
    pair_table$Mild_Untreated <- 0L
  }
  if (!"Mild_Treated" %in% colnames(pair_table)) {
    pair_table$Mild_Treated <- 0L
  }
  
  pair_subjects <- pair_table %>%
...
# =============================================================================
# ALL APECED VS HC
# Mild untreated + severe APECED subjects versus healthy controls
# Positive logFC / NES = higher in APECED
# =============================================================================
run_all_apeced_vs_hc <- function(ct, ct_counts, ct_meta, out_dir, gsea_dir) {
  contrast_name <- "AllAPECED_vs_HC"
  
  out_csv <- file.path(
    out_dir,
    paste0("DEG_", safe_name(ct), "_", contrast_name, ".csv")
  )
  
  meta_use <- ct_meta %>%
    dplyr::filter(group %in% c("Mild_Untreated", "Severe", "HC")) %>%
    dplyr::mutate(
      disease_group = dplyr::case_when(
        group == "HC" ~ "HC",
        group %in% c("Mild_Untreated", "Severe") ~ "APECED_All",
        TRUE ~ NA_character_
      )
    ) %>%
    dplyr::filter(!is.na(disease_group))
  
  group_counts <- table(meta_use$disease_group)
  
  if (
    !all(c("HC", "APECED_All") %in% names(group_counts)) ||
    any(group_counts[c("HC", "APECED_All")] < MIN_SAMPLES_GROUP)
  ) {
    write_skip(
      out_csv,
      paste0(
        "Insufficient samples for ", contrast_name,
        ". Counts: ",
  ...
  meta_use$disease_group <- factor(
    meta_use$disease_group,
    levels = c("HC", "APECED_All")
  )
  
  design <- model.matrix(~ 0 + disease_group, data = meta_use)
  colnames(design) <- sub("^disease_group", "", colnames(design))
  
  tt <- tryCatch(
    run_limma_contrast(
      counts_mat = ct_counts[, rownames(meta_use), drop = FALSE],
      design = design,
      contrast_formula = "APECED_All - HC",
      out_csv = out_csv
    ),
    error = function(e) {
      write_skip(out_csv, paste0("limma failed: ", e$message))
  ...
  deg_summary <- data.frame(
    celltype = ct,
    analysis = "all_apeced_vs_hc",
    contrast = contrast_name,
    n_up = sum(
      tt$adj.P.Val < DEG_FDR_CUTOFF &
        tt$logFC > DEG_LOGFC_CUTOFF,
      na.rm = TRUE
    ),
    n_down = sum(
      tt$adj.P.Val < DEG_FDR_CUTOFF &
    ...
    n_sig = sum(
      tt$adj.P.Val < DEG_FDR_CUTOFF &
        abs(tt$logFC) >= DEG_LOGFC_CUTOFF,
      na.rm = TRUE
    ),
    n_apeced = sum(meta_use$disease_group == "APECED_All"),
    n_hc = sum(meta_use$disease_group == "HC"),
    stringsAsFactors = FALSE
  )
  
  gsea_res <- run_gsea_for_deg(
    tt = tt,
    out_dir = gsea_dir,
    prefix = paste0(safe_name(ct), "_", contrast_name),
    top_n_plot = GSEA_TOP_N_PLOT
  )
  
  gsea_summary <- summarize_gsea_results(
    gsea_res = gsea_res,
    celltype = ct,
    analysis = "all_apeced_vs_hc",
    contrast = contrast_name
...
run_deg_gsea_pooled_and_within_pool <- function(obj,
                                                ...
                                                celltype_col = "Monaco_main_pruned",
                                                assay = "RNA",
                                                resume_from_celltype = NULL) {
  
  # Accept either:
  #   1) a loaded Seurat object, or
  #   2) a character path to an RDS file containing a Seurat object
  if (is.character(obj) && length(obj) == 1) {
    if (!file.exists(obj)) {
      stop("The supplied object path does not exist: ", obj)
    }
  ...
  if (!inherits(obj, "Seurat")) {
    stop(
      "`obj` must be a Seurat object or a path to an RDS containing one. ",
  ...
  package_versions <- data.frame(
    package = c(
      "R", "Seurat", "SeuratObject", "edgeR", "limma",
      "clusterProfiler", "ReactomePA", "msigdbr", "BiocParallel"
    ),
    version = c(
  ...
  data.table::fwrite(
    data.frame(
      setting = c("SCRIPT_VERSION", "REPRODUCIBLE_SEED", "BiocParallel"),
      value = c(SCRIPT_VERSION, as.character(REPRODUCIBLE_SEED), "SerialParam")
    ),
    file.path(out_dir, "reproducibility_settings.csv")
  )
  ...
  all_apeced_deg_dir  <- file.path(out_dir, "all_apeced_vs_hc", "deg_by_celltype")
  all_apeced_gsea_dir <- file.path(out_dir, "all_apeced_vs_hc", "gsea_by_celltype")
  ...
  dir.create(all_apeced_deg_dir, recursive = TRUE, showWarnings = FALSE)
  dir.create(all_apeced_gsea_dir, recursive = TRUE, showWarnings = FALSE)
  
  log_msg("DEG", "Section 4: Pseudobulk DEG + GSEA side-by-side")
  
  # Pool-level pseudobulk for pooled, within-pool, and paired analyses.
  pb <- build_pseudobulk(
    obj = obj,
  ...
  # Direct subject-level pseudobulk for mild-vs-HC and all-APECED-vs-HC.
  # Pool is excluded from sample_id, so all cells from the same
  # subject/group/cell type are summed during pseudobulk construction.
  pb_subject <- build_pseudobulk(
    obj = obj,
    celltype_col = celltype_col,
    assay = assay,
    min_cells_per_pb = MIN_CELLS_PER_PB,
  ...
  subject_counts <- pb_subject$counts
  subject_meta <- pb_subject$meta
  
  data.table::fwrite(
    sample_meta,
    file.path(out_dir, "pseudobulk_sample_metadata_pool_level.csv")
  )
  data.table::fwrite(
    subject_meta,
    file.path(out_dir, "pseudobulk_sample_metadata_subject_level.csv")
  )
  ...
  # Resume at the failed cell type without rerunning earlier cell types.
  if (!is.null(resume_from_celltype)) {
    start_index <- match(resume_from_celltype, celltypes)
    
    if (is.na(start_index)) {
      stop(
        "resume_from_celltype was not found. Available cell types are: ",
        paste(celltypes, collapse = ", ")
      )
    }
    
    celltypes <- celltypes[start_index:length(celltypes)]
    
    log_msg(
      "RESUME",
      paste0(
        "Resuming from cell type: ",
        resume_from_celltype,
        " (",
        length(celltypes),
  ...
  for (ct in celltypes) {
    ...
    # ------------------------------------------------------------------------
    # MATCHED SENSITIVITY ANALYSES USING DIRECT SUBJECT-LEVEL PSEUDOBULK
    # 1) Mild untreated APECED vs HC
    # 2) All APECED (mild untreated + severe) vs HC
    #
    # Both contrasts are oriented as APECED - HC:
    # positive logFC/NES = higher in APECED
    # ------------------------------------------------------------------------
    ct_all_meta <- subject_meta %>%
      dplyr::filter(
        CellType == ct,
        n_cells_pb >= MIN_CELLS_PER_PB
      )
    
    if (nrow(ct_all_meta) > 0) {
      ct_all_counts <- subject_counts[, rownames(ct_all_meta), drop = FALSE]
      
      ct_all_deg_dir <- file.path(all_apeced_deg_dir, ct_safe)
      ct_all_gsea_dir <- file.path(all_apeced_gsea_dir, ct_safe)
      
      dir.create(ct_all_deg_dir, recursive = TRUE, showWarnings = FALSE)
      dir.create(ct_all_gsea_dir, recursive = TRUE, showWarnings = FALSE)
      
      res9 <- run_unpaired_analysis(
        ct = ct,
        ct_counts = ct_all_counts,
        ct_meta = ct_all_meta,
        out_dir = ct_all_deg_dir,
        gsea_dir = ct_all_gsea_dir,
        analysis_label = "mild_only_vs_hc_subject_level",
        groups_keep = c("Mild_Untreated", "HC"),
        contrast_name = "MildUntreated_vs_HC_subject_level",
        contrast_formula = "groupMild_Untreated - groupHC",
        pool_subset = NULL
      )
      
      if (!is.null(res9$deg_summary)) {
        all_deg_summaries[[length(all_deg_summaries) + 1]] <- res9$deg_summary
      }
      if (!is.null(res9$gsea_summary)) {
        all_gsea_summaries[[length(all_gsea_summaries) + 1]] <- res9$gsea_summary
      }
      
      res10 <- run_all_apeced_vs_hc(
        ct = ct,
        ct_counts = ct_all_counts,
        ct_meta = ct_all_meta,
        out_dir = ct_all_deg_dir,
      ...
      if (!is.null(res10$deg_summary)) {
        all_deg_summaries[[length(all_deg_summaries) + 1]] <- res10$deg_summary
      }
      if (!is.null(res10$gsea_summary)) {
        all_gsea_summaries[[length(all_gsea_summaries) + 1]] <- res10$gsea_summary
      }
    }
  }
  ...
  invisible(list(
    pseudobulk_pool_level = pb,
    pseudobulk_subject_level = pb_subject,
    deg_summary = deg_summary,
    gsea_summary = gsea_summary
  ))
}
...
# Resume example:
# res_side_by_side <- run_deg_gsea_pooled_and_within_pool(
#   obj = "path/to/PBMC_object.rds",
#   out_dir = "deg_gsea_pooled_vs_within_pool",
#   celltype_col = "Monaco_main_pruned",
#   assay = "RNA",
#   resume_from_celltype = "Low-density neutrophils"
# )


# Full reproducible run:
res_side_by_side <- run_deg_gsea_pooled_and_within_pool(
  obj = "../PBMC_RDS/object_final_annotated.rds",
  out_dir = "deg_gsea_pooled_vs_within_pool",
  celltype_col = "Monaco_fine_pruned",
  assay = "RNA",
  resume_from_celltype = NULL
)
