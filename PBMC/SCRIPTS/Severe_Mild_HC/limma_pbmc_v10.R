##RERUN THIS SCRIPT BELOW

###############################################################################
# PSEUDOBULK DEG + GSEA SIDE-BY-SIDE
# CONSISTENT DISEASE-MINUS-REFERENCE VERSION
#
# Contrast direction rule:
#   - Mild_vs_HC           = Mild - HC
#   - Severe_vs_HC         = Severe - HC
#   - Severe_vs_Mild       = Severe - Mild
#   - MildTreated_vs_HC    = Mild Treated - HC
#   - Severe_vs_MildTreated= Severe - Mild Treated
#   - MildTreated_vs_MildUntreated = Mild Treated - Mild Untreated
#
# This makes downstream comparisons interpretable:
#   Mild_vs_HC      ↔ Severe_vs_HC
#   Mild_vs_HC      ↔ Severe_vs_Mild
#   Severe_vs_HC    ↔ Severe_vs_Mild
#
...
#
# Pool1 valid contrasts:
#   MildUntreated_vs_HC_Pool1
#   Severe_vs_HC_Pool1
#   Severe_vs_MildUntreated_Pool1
#
# Pool2 valid contrasts:
#   MildTreated_vs_HC_Pool2
#   Severe_vs_HC_Pool2
#   Severe_vs_MildTreated_Pool2
#
# Pooled valid contrasts:
#   MildUntreated_vs_HC_pooled
#   Severe_vs_HC_pooled
#   Severe_vs_MildUntreated_pooled
#   MildTreated_vs_HC_pooled
#   Severe_vs_MildTreated_pooled
#   MildTreated_vs_MildUntreated_pooled
#
# NOTE:
#   Severe_vs_MildUntreated_Pool2 is impossible (Mild Untreated not in Pool2)
###############################################################################
  ...
  if (!is.null(gene_list_entrez) && length(gene_list_entrez) >= 20) {
    ...
    if (!is.null(go_res)) {
      ...
      tryCatch(
        ...
        error = function(e) log_line("GO BP plot FAILED for ", prefix, ": ", e$message)
      )
      results$GO_BP <- res
    } else {
      write_skip(file.path(out_dir, paste0(prefix, "_GO_BP_SKIPPED.csv")), "GO BP returned NULL")
    }
  } else {
    log_line("SKIP GO BP: insufficient ENTREZ mapping")
    write_skip(file.path(out_dir, paste0(prefix, "_GO_BP_SKIPPED.csv")), "Insufficient ENTREZ mapping")
  }
  ...
  if (!is.null(gene_list_entrez) && length(gene_list_entrez) >= 20) {
    ...
    if (!is.null(reactome_res)) {
      ...
      tryCatch(
        ...
        error = function(e) log_line("Reactome plot FAILED for ", prefix, ": ", e$message)
      )
      results$Reactome <- res
    } else {
      write_skip(file.path(out_dir, paste0(prefix, "_Reactome_SKIPPED.csv")), "Reactome returned NULL")
    }
  } else {
    log_line("SKIP Reactome: insufficient ENTREZ mapping")
    write_skip(file.path(out_dir, paste0(prefix, "_Reactome_SKIPPED.csv")), "Insufficient ENTREZ mapping")
  }
  ...
  if (!is.null(hallmark_df)) {
    ...
    if (!is.null(hallmark_res)) {
      ...
      tryCatch(
        ...
        error = function(e) log_line("Hallmark plot FAILED for ", prefix, ": ", e$message)
      )
      results$Hallmark <- res
    } else {
      write_skip(file.path(out_dir, paste0(prefix, "_Hallmark_SKIPPED.csv")), "Hallmark returned NULL")
    }
  } else {
    write_skip(file.path(out_dir, paste0(prefix, "_Hallmark_SKIPPED.csv")), "Hallmark gene sets failed to load")
  }
  ...
  for (db in names(gsea_res)) {
    ...
    if (is.null(df) || nrow(df) == 0) {
      out[[length(out) + 1]] <- data.frame(
        celltype = celltype, analysis = analysis, contrast = contrast,
        database = db, n_sig = 0, stringsAsFactors = FALSE
      )
    } else {
      out[[length(out) + 1]] <- data.frame(
        celltype = celltype, analysis = analysis, contrast = contrast,
...
run_unpaired_analysis <- function(ct, ct_counts, ct_meta, out_dir, gsea_dir,
  ...
  gsea_res <- run_gsea_for_deg(tt, gsea_dir, paste0(safe_name(ct), "_", contrast_name),
...
run_deg_gsea_pooled_and_within_pool <- function(obj,
  ...
  for (d in c(pooled_deg_dir, pooled_gsea_dir, within_deg_dir, within_gsea_dir,
              paired_deg_dir, paired_gsea_dir)) {
    dir.create(d, recursive = TRUE, showWarnings = FALSE)
  }
  
  log_msg("DEG", "Section 4: Pseudobulk DEG + GSEA side-by-side")
  
  pb <- build_pseudobulk(obj = obj, celltype_col = celltype_col,
                         assay = assay, min_cells_per_pb = MIN_CELLS_PER_PB)
  ...
  for (ct in celltypes) {
    ...
    for (d in c(ct_pooled_deg_dir, ct_pooled_gsea_dir, ct_within_deg_dir,
                ct_within_gsea_dir, ct_paired_deg_dir, ct_paired_gsea_dir)) {
      dir.create(d, recursive = TRUE, showWarnings = FALSE)
    }
    ...
    # 1. Mild Untreated vs HC (pooled) = Mild - HC
    res1 <- run_unpaired_analysis(
      ct = ct, ct_counts = ct_counts, ct_meta = ct_meta,
      out_dir = ct_pooled_deg_dir, gsea_dir = ct_pooled_gsea_dir,
      analysis_label = "pooled",
      groups_keep = c("HC", "Mild_Untreated"),
      contrast_name = "MildUntreated_vs_HC_pooled",
    ...
    # 2. Severe vs HC (pooled) = Severe - HC
    ...
    # 3. Severe vs Mild Untreated (pooled) = Severe - Mild
    ...
    # 4. Mild Treated vs HC (pooled) = Mild Treated - HC
    res4 <- run_unpaired_analysis(
      ct = ct, ct_counts = ct_counts, ct_meta = ct_meta,
      out_dir = ct_pooled_deg_dir, gsea_dir = ct_pooled_gsea_dir,
      analysis_label = "pooled",
      groups_keep = c("HC", "Mild_Treated"),
      contrast_name = "MildTreated_vs_HC_pooled",
      contrast_formula = "groupMild_Treated - groupHC"
    )
    ...
    # 5. Severe vs Mild Treated (pooled) = Severe - Mild Treated
    res5 <- run_unpaired_analysis(
      ct = ct, ct_counts = ct_counts, ct_meta = ct_meta,
      out_dir = ct_pooled_deg_dir, gsea_dir = ct_pooled_gsea_dir,
      analysis_label = "pooled",
      groups_keep = c("Mild_Treated", "Severe"),
      contrast_name = "Severe_vs_MildTreated_pooled",
      contrast_formula = "groupSevere - groupMild_Treated"
    )
    ...
    # 6. Mild Treated vs Mild Untreated (pooled)
    res6 <- run_unpaired_analysis(
      ct = ct, ct_counts = ct_counts, ct_meta = ct_meta,
      out_dir = ct_pooled_deg_dir, gsea_dir = ct_pooled_gsea_dir,
      analysis_label = "pooled",
      groups_keep = c("Mild_Untreated", "Mild_Treated"),
      contrast_name = "MildTreated_vs_MildUntreated_pooled",
      contrast_formula = "groupMild_Treated - groupMild_Untreated"
    )
    ...
    # 7. Mild Untreated vs HC (Pool1) = Mild - HC
    res7 <- run_unpaired_analysis(
      ct = ct, ct_counts = ct_counts, ct_meta = ct_meta,
      out_dir = ct_within_deg_dir, gsea_dir = ct_within_gsea_dir,
      analysis_label = "within_pool1",
      groups_keep = c("HC", "Mild_Untreated"),
      contrast_name = "MildUntreated_vs_HC_Pool1",
      contrast_formula = "groupMild_Untreated - groupHC",
      pool_subset = "Pool1"
    )
    ...
    # 8. Severe vs HC (Pool1) = Severe - HC
    res8 <- run_unpaired_analysis(
      ct = ct, ct_counts = ct_counts, ct_meta = ct_meta,
      out_dir = ct_within_deg_dir, gsea_dir = ct_within_gsea_dir,
      analysis_label = "within_pool1",
      groups_keep = c("HC", "Severe"),
    ...
    # 9. Severe vs Mild Untreated (Pool1) = Severe - Mild
    ...
    if (!is.null(res9$deg_summary))  all_deg_summaries[[length(all_deg_summaries) + 1]]   <- res9$deg_summary
    if (!is.null(res9$gsea_summary)) all_gsea_summaries[[length(all_gsea_summaries) + 1]] <- res9$gsea_summary
    
    # -------------------------------------------------------------------------
    # WITHIN POOL2
    # -------------------------------------------------------------------------
    
    # 10. Mild Treated vs HC (Pool2) = Mild Treated - HC
    res10 <- run_unpaired_analysis(
      ct = ct, ct_counts = ct_counts, ct_meta = ct_meta,
      out_dir = ct_within_deg_dir, gsea_dir = ct_within_gsea_dir,
      analysis_label = "within_pool2",
      groups_keep = c("HC", "Mild_Treated"),
      contrast_name = "MildTreated_vs_HC_Pool2",
      contrast_formula = "groupMild_Treated - groupHC",
      pool_subset = "Pool2"
    )
    if (!is.null(res10$deg_summary))  all_deg_summaries[[length(all_deg_summaries) + 1]]   <- res10$deg_summary
    if (!is.null(res10$gsea_summary)) all_gsea_summaries[[length(all_gsea_summaries) + 1]] <- res10$gsea_summary
    
    # 11. Severe vs HC (Pool2) = Severe - HC
    res11 <- run_unpaired_analysis(
      ct = ct, ct_counts = ct_counts, ct_meta = ct_meta,
      out_dir = ct_within_deg_dir, gsea_dir = ct_within_gsea_dir,
      analysis_label = "within_pool2",
      groups_keep = c("HC", "Severe"),
    ...
    if (!is.null(res11$deg_summary))  all_deg_summaries[[length(all_deg_summaries) + 1]]   <- res11$deg_summary
    if (!is.null(res11$gsea_summary)) all_gsea_summaries[[length(all_gsea_summaries) + 1]] <- res11$gsea_summary
    
    # 12. Severe vs Mild Treated (Pool2) = Severe - Mild Treated
    res12 <- run_unpaired_analysis(
      ct = ct, ct_counts = ct_counts, ct_meta = ct_meta,
      out_dir = ct_within_deg_dir, gsea_dir = ct_within_gsea_dir,
      analysis_label = "within_pool2",
      groups_keep = c("Mild_Treated", "Severe"),
      contrast_name = "Severe_vs_MildTreated_Pool2",
      contrast_formula = "groupSevere - groupMild_Treated",
      pool_subset = "Pool2"
    )
    if (!is.null(res12$deg_summary))  all_deg_summaries[[length(all_deg_summaries) + 1]]   <- res12$deg_summary
    if (!is.null(res12$gsea_summary)) all_gsea_summaries[[length(all_gsea_summaries) + 1]] <- res12$gsea_summary
    
    # -------------------------------------------------------------------------
    # PAIRED CROSS-POOL: Mild Treated (Pool2) vs Mild Untreated (Pool1)
    # -------------------------------------------------------------------------
    res13 <- tryCatch(
      run_paired_analysis(
        ct = ct, ct_counts = ct_counts, ct_meta = ct_meta,
        out_dir = ct_paired_deg_dir, gsea_dir = ct_paired_gsea_dir
      ),
      error = function(e) {
    ...
    if (!is.null(res13)) {
      if (!is.null(res13$deg_summary))  all_deg_summaries[[length(all_deg_summaries) + 1]]   <- res13$deg_summary
      if (!is.null(res13$gsea_summary)) all_gsea_summaries[[length(all_gsea_summaries) + 1]] <- res13$gsea_summary
    }
  }
...
res_side_by_side <- run_deg_gsea_pooled_and_within_pool(
  obj          = obj_filtered,
  out_dir      = "deg_gsea_pooled_vs_within_pool_Monaco_fine_pruned_v3_consistent_disease_direction",
  celltype_col = "Monaco_fine_pruned",
  assay        = "RNA"
)

# To resume from a specific cell type:
# res_side_by_side <- run_deg_gsea_pooled_and_within_pool(
#   obj                 = obj_filtered,
#   out_dir             = "deg_gsea_pooled_vs_within_pool_Monaco_fine_pruned_v3_consistent_disease_direction",
#   celltype_col        = "Monaco_fine_pruned",
